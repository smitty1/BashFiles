
#Apply color to man pages.
#From https://wiki.archlinux.org/index.php/Man_Page#Colored_man_pages
man() {
        env \
                LESS_TERMCAP_mb=$(printf "\e[1;31m") \
                LESS_TERMCAP_md=$(printf "\e[1;31m") \
                LESS_TERMCAP_me=$(printf "\e[0m") \
                LESS_TERMCAP_se=$(printf "\e[0m") \
                LESS_TERMCAP_so=$(printf "\e[1;44;33m") \
                LESS_TERMCAP_ue=$(printf "\e[0m") \
                        man "$@"
}

# cd and ls in one
# From https://wiki.archlinux.org/index.php/Bash
cl() {
    if [[ -d "$1" ]]; then
        cd "$1"
        ls -lAhF --color=auto --group-directories-first
    else
        echo "bash: cl: '$1': Directory not found"
    fi
}

     
# Simple note taker 
# From https://wiki.archlinux.org/index.php/Bash
note () {
    # if file doesn't exist, create it
    if [[ ! -f /home/jimsmith/.notes ]]; then
        touch /home/jimsmith/.notes
    fi

    if [[ $# -eq 0 ]]; then
        # no arguments, print file
        cat /home/jimsmith/.notes
    elif [[ "$1" == "-c" ]]; then
        # clear file
        echo "" > /home/jimsmith/.notes
    else
        # add all arguments to file
        echo "$@" >> /home/jimsmith/.notes
    fi
}



#Simple task utility
#Inspired by #Simple note taker 
# From https://wiki.archlinux.org/index.php/Bash

todo() {
    if [[ ! -f /home/jimsmith/.todo ]]; then
        touch /home/jimsmith/.todo
    fi

    if [[ $# -eq 0 ]]; then
        cat /home/jimsmith/.todo
    elif [[ "$1" == "-l" ]]; then
        cat -n /home/jimsmith/.todo
    elif [[ "$1" == "-c" ]]; then
        echo "" > /home/jimsmith/.todo
    elif [[ "$1" == "-r" ]]; then
        cat -n /home/jimsmith/.todo
        echo -ne "----------------------------\nType a number to remove: "
        read NUMBER
        sed -ie ${NUMBER}d /home/jimsmith/.todo
    else
        echo "$@" >> /home/jimsmith/.todo
    fi
}


 ###### dnf-history function ###################
function dnf-history(){
      case "$1" in
        install)
              cat /var/log/dnf.log | grep 'install '
              ;;
        upgrade|remove)
              cat /var//log/dnf.log | grep $1
              ;;
        rollback)
              cat /var/log/dnf.log | grep upgrade | \
                  grep "$2" -A10000000 | \
                  grep "$3" -B10000000 | \
                  awk '{print $4"="$5}'
              ;;
        *)
              cat /var/log/dnf.log
              ;;
      esac
}

# ex - archive extractor
# usage: ex <file>
ex ()
{
  if [ -f $1 ] ; then
    case $1 in
      *.tar.bz2)   tar xjf $1   ;;
      *.tar.gz)    tar xzf $1   ;;
      *.bz2)       bunzip2 $1   ;;
      *.rar)       unrar x $1     ;;
      *.gz)        gunzip $1    ;;
      *.tar)       tar xf $1    ;;
      *.tbz2)      tar xjf $1   ;;
      *.tgz)       tar xzf $1   ;;
      *.zip)       unzip $1     ;;
      *.Z)         uncompress $1;;
      *.7z)        7z x $1      ;;
      *)           echo "'$1' cannot be extracted via ex()" ;;
    esac
  else
    echo "'$1' is not a valid file"
  fi
}


#Use one command to make a directory  and change into it.
mkdircd(){ mkdir "$1" && cd "$1" ; }


genpasswd() { 
    strings /dev/urandom | grep -o '[[:alnum:]]' | head -n 14 | tr -d '\n'; echo
}



#Sort the body of output, while leaving the header on line 1 intact

body() {
  IFS= read -r header
  printf '%s\n' "$header"
  "$@"
}

function now () {
        echo "It's $(date +%R)"
} 

#Lsit all Symbolic Links in the current directory             
                                                              
lks() {                                                       
        ls -CFlah --group-directories-first | grep ^l         
} 


#Restore control of the grub login screen to the current distribution
#Renew_Grub() {
	#	sudo grub-install /dev/sda
	#	sudo update-grub
#}
                                                                                                                          
ipif() {                                                      
    if grep -P "(([1-9]\d{0,2})\.){3}(?2)" <<< "$1"; then     
         curl ipinfo.io/"$1"                                  
    else                                                      
        ipawk=($(host "$1" | awk '/address/ { print $NF }'))  
        curl ipinfo.io/${ipawk[1]}                            
    fi                                                        
    echo                                                      
}                                                               

